...
if [ $logf_enabled == "0" ]
then /etc/zabbix/zabbix_agentd.d/read-apache-logs.py --last hour "$filepath" | grep '/ HTTP/1.1" 20' | awk '{print $4}' | sort | uniq | wc -l
else /etc/zabbix/zabbix_agentd.d/read-apache-logs.py --last hour "$filepath" | grep '/ HTTP/1.1" 20' | awk '{print $3}' | sort | uniq | wc -l
fi
